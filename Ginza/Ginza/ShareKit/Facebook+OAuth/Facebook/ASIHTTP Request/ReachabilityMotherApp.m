...
@implementation ReachabilityMotherApp
...
+ (ReachabilityMotherApp *)sharedReachability
{
    if (!_sharedReachability)
	 {
        _sharedReachability = [[ReachabilityMotherApp alloc] init];
...
- (void)scheduleOnRunLoop:(NSRunLoop *)inRunLoop
{
    ...
    if ([[ReachabilityMotherApp sharedReachability] networkStatusNotificationsEnabled] == NO) {
        return;
    }
